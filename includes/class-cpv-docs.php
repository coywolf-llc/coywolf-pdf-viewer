 ...
 */
final class Coywolf_CPV_Docs {

	/**
	 * Output the Documentation admin page.
	 */
	public static function render_page() {
		echo '<div class="wrap coywolf-cpv-docs">';

		$markdown = self::read_readme();
		if ( '' === $markdown ) {
			echo '<h1>' . esc_html__( 'Documentation', 'coywolf-pdf-viewer' ) . '</h1>';
			echo '<p>' . esc_html__( 'The documentation could not be loaded.', 'coywolf-pdf-viewer' ) . '</p>';
			echo '</div>';
			return;
		}
	...
	private static function read_readme() {
		$file = COYWOLF_CPV_PATH . 'readme.md';
	...
	public static function to_html( $md ) {
		...
		while ( $i < $n ) {
			...
			if ( preg_match( '/^```\s*([A-Za-z0-9_-]*)\s*$/', $line, $m ) ) {
				...
				$out .= '<pre class="coywolf-cpv-docs-code"><code' . $cls . '>' . esc_html( implode( "\n", $buf ) ) . '</code></pre>';
				continue;
			}
			...
			if ( false !== strpos( $line, '|' ) && isset( $lines[ $i + 1 ] )
				...
				$out .= '<table class="coywolf-cpv-docs-table widefat striped"><thead><tr>';
